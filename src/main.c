...
static bool resize_textures(int render_width, int render_height) {
	if (depth_texture) {
...
static void draw_opaque() {
	...
	world_render(&player_camera, commands, pass, CHUNK_TYPE_OPAQUE, 0);
	SDL_EndGPURenderPass(pass);
}
...
static void commit(const float dt) {
	cooldown += dt;
